...
def write_html(results, sentinel, path, stats, ml_stats: dict = None):
    ...
    def _build_ml_panel(ml_stats, hl):
        if not ml_stats:
            return ""
        ...
        html = (
            ...
            + mk(b_delta,                    "Brain Age Delta",  "")
            + "</div>"
            "<p style='color:#6e7681;font-size:.78rem;margin-top:.5rem'>"
            "⚠️ ML confidence is LOW for MRA source images (not optimised for tissue contrast). "
            "Results augment AI visual analysis — dedicated T1 MPRAGE + FLAIR + coronal T1 "
            "required for clinical-grade structural assessment. "
...
def main():
    ...
    # ── Delta comparison from MLflow ───────────────────────────────────────────
    if ml_client and ml_agg["tissue"]:
        print("\n📊 MLflow delta vs prior run:")
        try:
            import mlflow
            ...
            if len(all_runs) >= 2:
                ...
                prev = all_runs.iloc[1]
                for metric in ["grey_matter_pct","wmh_pct_of_wm","mta_proxy_left","atrophy_grade"]:
                    ck = f"metrics.{metric}"
                    pk = f"metrics.{metric}"
                    if ck in curr and pk in prev:
                        delta = float(curr[ck]) - float(prev[pk])
                        arrow = "↑" if delta > 0.01 else ("↓" if delta < -0.01 else "→")
                        print(f"  {metric}: {arrow} {delta:+.3f}")
            else:
                print("  (first run — no prior data for comparison)")
        except Exception as e:
            print(f"  (MLflow delta error: {e})")
