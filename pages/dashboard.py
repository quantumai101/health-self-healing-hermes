...
def render() -> None:
    ...
    try:
        ...
        # Log the full stack trace to server-side logs
        logger.error(f"Dashboard rendering error: {traceback.format_exc()}")
        # Show only a generic message to the user
        st.error("❌ An unexpected error occurred while loading the dashboard. Please contact support if the issue persists.")
...
def _render_inner() -> None:
    ...
    def _load_df():
        """
        Fetches and processes patient data. Cached for 1 hour to optimize performance.
        """
        df = get_all_patients()
