...
Fix (2026-06-12):
  * Disclaimer modal margin-top reduced so button is always visible
  * 7th CTCA button navigates to CTCA Panel View page instead of
    calling NEXUS agent (which returned a text summary, not the viewer)
  * _render_message routes HTML through st.components.v1.html()
...
# Index of the CTCA navigation button in SUGGESTED_OPERATIONS (0-based)
_CTCA_NAV_INDEX = 6
...
def _extract_parts(text: str):
    match_start = re.search(
        r'<div\s+id=["\']nexus-|<canvas\s|<(div|script)\b', text, re.IGNORECASE
    )
    if not match_start:
        return text, None, ""
...
def _inject_watermark(html: str) -> str:
    if re.search(r'</body>', html, re.IGNORECASE):
        return re.sub(
            r'</body>', _SIMULATION_WATERMARK + '</body>', html, flags=re.IGNORECASE
        )
    ...
    try:
        with st.chat_message(role):
            if _is_html_content(text):
                ...
                if html_block:
                    components.html(
                        _inject_watermark(html_block),
                        height=_CTCA_VIEWER_HEIGHT,
                        scrolling=False,
                    )
                if post.strip():
                    st.markdown(post)
...
# Each entry: (button_label, agent_name_or_None, command_string)
# agent_name = None means "navigate to page" instead of calling an agent
SUGGESTED_OPERATIONS = [
    ("Ingest synthetic patient biometric data",
     "AXIOM",
     "Ingest synthetic patient biometric data and run clinical quality checks"),
    ("Train XGBoost chronic disease risk model",
     "PROMETHEUS",
     "Train the XGBoost chronic disease risk model and show AUC score"),
    ("Run population health digital twin",
     "NEXUS",
    ...
    ("Run N-1 CTCA digital twin simulation",
     "NEXUS",
     "Run N-1 CTCA digital twin simulation"),
...
def _render_suggested_ops():
    st.markdown(
        ...
        "margin-bottom:8px;'>✦ SUGGESTED HEALTH OPERATIONS</p>",
    ...
    for i, (label, agent_name, command) in enumerate(SUGGESTED_OPERATIONS):
        with cols[i % 2]:
            if st.button(label, key=f"chat_sugg_{i}", use_container_width=True):
                ...
                # 7th button: navigate to CTCA Panel View page
                if agent_name is None:
                    st.session_state["page_selection"] = command
                    st.rerun()
                    return
