...
# ─── Scientific ML Skills (K-Dense-style SDK + MLflow) ───────────────────────
try:
    from brain_ml_skills import BrainSkillClient
    HAS_ML_SKILLS = True
except ImportError:
    HAS_ML_SKILLS = False
    print("⚠️  brain_ml_skills.py not found — ML analysis disabled. "
          "Place brain_ml_skills.py in the same folder as this script.")
...
MLFLOW_DB  = str(OUTPUT_DIR / "brain_mlflow.db")   # MLflow experiment tracking DB
...
# ── Auto-calculate patient age from DOB + scan date ──────────────────────────
def _calc_age(dob_str: str, scan_str: str) -> int:
    """Calculate exact age at time of scan from DOB and scan date strings."""
    from datetime import datetime
    ...
    for fmt in ("%d %B %Y", "%d/%m/%Y", "%d %b %Y", "%Y-%m-%d"):
        try:
            scan = datetime.strptime(scan_str.strip(), fmt)
            break
        except ValueError:
            continue
    else:
...
_DOB       = "14/03/1955"
_SCAN_DATE = "28 May 2022"      # ← change this for each new scan
_SCAN_AGE  = _calc_age(_DOB, _SCAN_DATE)

PATIENT = dict(
    name="Zhang Zhi Ming", dob=_DOB, age=_SCAN_AGE, sex="Male",
    scan_date=_SCAN_DATE, facility="MRA Brain 28May2022",
    known_history=(
...
print(f"  📅 Age at scan ({_SCAN_DATE}): {_SCAN_AGE} years  [DOB: {_DOB}]")

# ─── Series routing — MRA-focused (TOF / vessels are highest priority) ─────────
# Agents: AXIOM=vessel/structural, NOVA=report, HERMES=coordinator
SERIES_ROUTES = {
    # ── MRA / Vessel sequences (AXIOM agent — priority 3) ──────────────────────
    ...
    "INTRACRAN":    dict(priority=3, agent="AXIOM", role="vessels"),
    "CIRCLE":       dict(priority=3, agent="AXIOM", role="vessels"),
    "VESSEL":       dict(priority=3, agent="AXIOM", role="vessels"),
    "MIP":          dict(priority=3, agent="AXIOM", role="mip"),
    "3D_MRA":       dict(priority=3, agent="AXIOM", role="vessels"),
    # ── WMH / FLAIR sequences (AXIOM agent — priority 2) ─────────────────────
    "T2_FLAIR":     dict(priority=2, agent="AXIOM", role="wmh"),
    "FLAIR":        dict(priority=2, agent="AXIOM", role="wmh"),
    "T2FLAIR":      dict(priority=2, agent="AXIOM", role="wmh"),
    # ── Structural T1 (AXIOM agent — priority 2) ──────────────────────────────
    "T1_TIRM":      dict(priority=2, agent="AXIOM", role="t1volume"),
    "TIRM":         dict(priority=2, agent="AXIOM", role="t1volume"),
    "MPRAGE":       dict(priority=2, agent="AXIOM", role="t1volume"),
    "T1_MPRAGE":    dict(priority=2, agent="AXIOM", role="t1volume"),
    "BRAVO":        dict(priority=2, agent="AXIOM", role="t1volume"),
    # ── T2 Structural (AXIOM agent — priority 1) ──────────────────────────────
    "T2_TSE":       dict(priority=1, agent="AXIOM", role="t2struct"),
    "T2_SPACE":     dict(priority=1, agent="AXIOM", role="t2struct"),
    "T2W":          dict(priority=1, agent="AXIOM", role="t2struct"),
    # ── DWI / Diffusion (AXIOM agent — priority 2) ────────────────────────────
    "DWI":          dict(priority=2, agent="AXIOM", role="dwi"),
    "DTI":          dict(priority=2, agent="AXIOM", role="dwi"),
    "EP2D_DIFF":    dict(priority=2, agent="AXIOM", role="dwi"),   # ← Siemens DWI
    "DIFF":         dict(priority=2, agent="AXIOM", role="dwi"),
    "TRACE":        dict(priority=2, agent="AXIOM", role="dwi"),
    "ADC":          dict(priority=2, agent="AXIOM", role="dwi"),
    # ── Report PDFs (NOVA agent) ───────────────────────────────────────────────
    "REPORT":       dict(priority=2, agent="NOVA",  role="pdf_report"),
    "SCOUT":        dict(priority=0, agent="AXIOM", role="t2struct"),  # low priority
    "LOCALIZER":    dict(priority=0, agent="AXIOM", role="t2struct"),
}

# Keywords whose series also get a structural-brain second pass
MRA_STRUCTURAL_ALSO = {"TOF_3D", "TOF", "MRA", "ANGIO", "INTRACRAN", "3D_MRA", "VESSEL"}

# ── HERMES AGENT MANIFEST ─────────────────────────────────────────────────────
# Documents which agent handles which role for logging/display
AGENT_MANIFEST = {
    "AXIOM":  "Vessel & Structural Brain Analysis (MRA/T1/T2/DWI/FLAIR)",
    "NOVA":   "Report & PDF Extraction Agent",
    "HERMES": "Coordinator — routes series, aggregates SENTINEL report",
}
...
# ─────────────────────────────────────────────
def analyse(client, series, ml_client=None):
    """
    Analyse one DICOM series via Gemini AI.
    If ml_client (BrainSkillClient) is provided, runs pixel-level ML analysis
    first and injects the quantitative results into the Gemini prompt so the
    AI can use real pixel measurements rather than visual estimates.
    """
    role   = series["role"]
    ...
    # ── Step 1: Run ML pixel analysis (if available) ─────────────────────────
    ml_result     = None
    ml_context    = ""
    ml_result_dict= {}
    if ml_client and role in ("mra_structural", "vessels", "wmh", "t1volume", "t2struct", "dwi"):
        try:
            ml_result = ml_client.analyse_series(
                dicom_paths = images,
                series_desc = series["desc"],
                patient_age = PATIENT["age"],
                max_slices  = 12,
            )
            ml_context     = ml_result.to_gemini_context()
            ml_result_dict = ml_result.to_dict()
            print(f"  🧠 ML: GM={ml_result.tissue.grey_matter_pct}% "
                  f"WM={ml_result.tissue.white_matter_pct}% "
                  f"WMH={ml_result.wmh.wmh_pct_of_wm:.2f}% "
                  f"MTA_L={ml_result.mta.mta_proxy_left} "
                  f"[{ml_result.overall_confidence} confidence]")
        except Exception as e:
            print(f"  ⚠️  ML analysis error: {e}")

    # ── Step 2: Build Gemini prompt — inject ML measurements ─────────────────
    base_prompt = get_prompt(role)
    if ml_context:
        prompt = (
            base_prompt +
            f"\n\n{ml_context}\n\n"
            "INSTRUCTION: The ML_SKILL_RESULTS above are pixel-level measurements "
            "computed by a scientific segmentation algorithm (GMM + Otsu). "
            "Use these numbers directly to populate the numeric fields in your JSON response. "
            "Do NOT override them with visual guesses — incorporate them as your primary data source. "
            "Add your visual interpretation in the findings_summary and notes fields."
        )
    else:
        prompt = base_prompt

    # ── Step 3: Build image parts ─────────────────────────────────────────────
    ...
    # ── Step 5: Merge ML measurements into result ─────────────────────────────
    if ml_result_dict:
        result["_ml_pixel_analysis"] = {
            "tissue":    ml_result_dict.get("tissue", {}),
            "wmh":       ml_result_dict.get("wmh", {}),
            "mta":       ml_result_dict.get("mta", {}),
            "morph":     ml_result_dict.get("morph", {}),
            "confidence": ml_result_dict.get("overall_confidence", "low"),
            "mlflow_run_id": ml_result_dict.get("run_id", ""),
            "elapsed_s":    ml_result_dict.get("elapsed_s", 0),
        }
...
# ─────────────────────────────────────────────
def sentinel_summary(client, results, ml_summary: str = ""):
    try:
...
{"ML PIXEL ANALYSIS (from BrainSkillClient — use these precise numbers first):" + chr(10) + ml_summary if ml_summary else ""}

ALL FINDINGS (Gemini AI visual analysis): {findings}"""
...
def write_txt(results, sentinel, path, stats):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write(f"  Series: {stats['series_ok']}/{stats['series_total']} | "
                ...
                f"ML series: {stats.get('ml_series', 0)}\n")
        f.write(f"  MLflow DB: {MLFLOW_DB}\n")
        f.write("="*70+"\n\n"+sentinel+"\n\n")
...
# ─────────────────────────────────────────────
def write_html(results, sentinel, path, stats, ml_stats: dict = None):
    ...
    # ── Build ML Skills panel HTML ────────────────────────────────────────────
    ml_stats = ml_stats or {}

    # Clinical confidence descriptions (patient-friendly)
    CONF_LABELS = {
        "high":     "Quantitative",
    ...
    def _build_ml_panel(ml_stats, hl, conf_labels=None):
        conf_labels = conf_labels or CONF_LABELS
        if not ml_stats:
            return ""
        t  = ml_stats.get("tissue",  {})
        w  = ml_stats.get("wmh",     {})
        m  = ml_stats.get("mta",     {})
        mo = ml_stats.get("morph",   {})
        run_id   = ml_stats.get("mlflow_run_id", "")
        conf_all = ml_stats.get("overall_confidence", "low")
        db_path  = ml_stats.get("mlflow_db", "brain_mlflow.db")
        n_runs   = ml_stats.get("total_runs", 0)
        n_slices = ml_stats.get("n_slices_total", 0)
        conf_cls = "conf-" + conf_all if conf_all in ("low","moderate","high") else "conf-low"

        def mk(val, label, conf=""):
            cs = ("<div class='mc'>" + hl.escape(str(conf)) + " confidence</div>") if conf else ""
            return ("<div class='ml-metric'>"
                    "<div class='mv'>" + hl.escape(str(val)) + "</div>"
                    "<div class='ml'>" + hl.escape(label) + "</div>" + cs + "</div>")

        run_badge = ("<span class='mlflow-badge'>run:" + hl.escape(run_id[:8]) + "...</span>"
                     if run_id else "<span class='mlflow-badge'>not logged</span>")

        gm  = str(t.get("grey_matter_pct",  "N/A"))
        wm  = str(t.get("white_matter_pct", "N/A"))
        csf = str(t.get("csf_pct",          "N/A"))
        gmr = str(t.get("gm_wm_ratio",      "N/A"))
        tc  = CONF_LABELS.get(t.get("confidence","low"), t.get("confidence","low"))
        wmh_pct = str(w.get("wmh_pct_of_wm",           "N/A"))
        faz_pv  = str(w.get("fazekas_periventricular",  "N/A"))
        faz_d   = str(w.get("fazekas_deep",             "N/A"))
        burden  = str(w.get("burden",                   "N/A"))
        lac     = str(w.get("lacune_candidate_count",   "N/A"))
        wc      = CONF_LABELS.get(w.get("confidence","low"), w.get("confidence","low"))
        mta_l   = str(m.get("mta_proxy_left",           "N/A"))
        mta_r   = str(m.get("mta_proxy_right",          "N/A"))
        asym    = str(m.get("asymmetry_pct",            "N/A"))
        hoc_l   = str(m.get("hoc_estimate_left",        "N/A"))
        hoc_r   = str(m.get("hoc_estimate_right",       "N/A"))
        mc      = CONF_LABELS.get(m.get("confidence","low"), m.get("confidence","low"))
        grade   = str(mo.get("atrophy_grade",           "N/A"))
        sulc    = str(mo.get("sulcal_widening",         "N/A"))
        vent    = str(mo.get("ventricular_size",        "N/A"))
        b_delta_raw = mo.get("brain_age_delta", None)
        b_delta = (f"{float(b_delta_raw):+.1f} yr" if b_delta_raw is not None else "N/A")
        moc     = CONF_LABELS.get(mo.get("confidence","low"), mo.get("confidence","low"))
        method  = str(t.get("method",""))[:120]

        html = (
            "<div class='ml-panel'>"
            "<h2>🧬 Scientific ML Analysis — BrainSkillClient v1.0 + MLflow</h2>"
            "<p style='color:#8b949e;font-size:.85rem;margin-bottom:.75rem'>"
            "Pixel-level quantitative analysis using Gaussian Mixture Model (GMM) segmentation, "
            "multi-Otsu thresholding, and morphological analysis across "
            + hl.escape(str(n_slices)) + " DICOM series. "
            "Overall confidence: <strong class='" + conf_cls + "'>" + hl.escape(conf_all.upper()) + "</strong>"
            " &nbsp;|&nbsp; MLflow DB: <code>" + hl.escape(db_path) + "</code>"
            " &nbsp;|&nbsp; " + run_badge +
            " &nbsp;|&nbsp; Total runs logged: " + str(n_runs) +
            "</p>"
            "<div class='ml-grid'>"
            + mk(gm  + "%",                  "Grey Matter %",    tc)
            + mk(wm  + "%",                  "White Matter %",   tc)
            + mk(csf + "%",                  "CSF %",            tc)
            + mk(gmr,                        "GM/WM Ratio",      "")
            + mk(wmh_pct + "% of WM",        "WMH Burden %",     wc)
            + mk("PV=" + faz_pv + " D=" + faz_d, "Fazekas Grade", "")
            + mk(burden,                     "WMH Classification","")
            + mk(lac,                        "Lacune Candidates", "")
            + mk("L=" + mta_l + " R=" + mta_r, "MTA Proxy (0–4)", mc)
            + mk(asym + "%",                 "MTA Asymmetry",    "")
            + mk("L=" + hoc_l + "% R=" + hoc_r + "%", "HOC Estimate", "")
            + mk("Grade " + grade + "/3",    "Cortical Atrophy", moc)
            + mk(sulc,                       "Sulcal Widening",  "")
            + mk(b_delta,                    "Brain Age vs Scan-Age (' + str(PATIENT['age']) + ' yr)",  "")
            + "</div>"
            "<p style='color:#6e7681;font-size:.78rem;margin-top:.5rem'>"
            "<strong>ℹ️ How to read this panel:</strong><br>"
            "• <strong>Grey/White Matter %</strong>: Proportion of brain tissue in each compartment "
            "(normal age ' + str(PATIENT['age']) + ': GM ~42–46%, WM ~40–46%, CSF ~10–16%).<br>"
            "• <strong>WMH Burden %</strong>: White matter bright spots — small vessel disease marker. Normal &lt;2%.<br>"
            "• <strong>MTA Proxy</strong>: Medial Temporal Atrophy score (0=none, 4=severe). Normal ≤1.5 for your age.<br>"
            "• <strong>HOC Estimate</strong>: Hippocampal Occupancy — memory structure volume. Normal &gt;60%.<br>"
            "• <strong>Brain Age vs Scan-Age</strong>: +9yr means brain tissue morphology (sulcal width, "
            "CSF spaces) resembles a person ~9 years older. Negative = younger-appearing. "
            "This is an estimate from MRA background — confirm with T1 MRI.<br>"
            "• <strong>Measurement source</strong>: MRA is a vessel study — tissue confidence is approximate. "
            "Dedicated T1 MPRAGE + FLAIR + coronal T1 MRI gives clinical-grade accuracy.<br>"
            "Method: " + hl.escape(method) +
            "</p></div>"
        )
        return html

    ml_panel_html = _build_ml_panel(ml_stats, hl)
...
<div class='card card-error'>
  ...
  <em>Re-run the script — retry logic will attempt recovery.</em></div>
        ...
        for k, v in r.items():
            ...
            # Mark ML pixel-sourced metrics with a green ML tag
            ML_SOURCED = {
                "grey_matter_pct","white_matter_pct","csf_pct","gm_wm_ratio",
                "wmh_pct_of_wm","fazekas_periventricular","fazekas_deep",
                "mta_proxy_left","mta_proxy_right","atrophy_grade",
                "sulcal_csf_fraction","ventricular_csf_frac","brain_age_delta",
                "hoc_estimate_left","hoc_estimate_right","asymmetry_pct",
            }
            is_ml = k in ML_SOURCED or k == "_ml_pixel_analysis"
            ml_tag = " <span class='ml-source-tag'>ML</span>" if is_ml else ""
            tr_cls = "ml-card-row" if is_ml else ("highlight-row" if is_highlight else "")
            tr_open = f"<tr class='{tr_cls}'>" if tr_cls else "<tr>"
            rows += (f"{tr_open}"
                     f"<td class='key-col'>{hl.escape(k)}{ml_tag}</td>"
...
/* ── ML Skills Panel ── */
.ml-panel{{background:#0d1f0d;border:1px solid #3fb95055;border-radius:12px;padding:1.5rem;margin-bottom:1.5rem}}
.ml-panel h2{{color:#3fb950;border-color:#3fb95033}}
.ml-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:.75rem;margin:.75rem 0}}
.ml-metric{{background:#0a1a0a;border:1px solid #3fb95033;border-radius:8px;padding:.75rem;text-align:center}}
.ml-metric .mv{{font-size:1.5rem;font-weight:700;color:#3fb950}}
.ml-metric .ml{{font-size:.75rem;color:#8b949e;margin-top:.2rem}}
.ml-metric .mc{{font-size:.7rem;color:#d29922;margin-top:.1rem}}
.conf-low{{color:#d29922}} .conf-moderate{{color:#3fb950}} .conf-high{{color:#58a6ff}}
.mlflow-badge{{display:inline-block;background:#1a2a3a;color:#58a6ff;border:1px solid #58a6ff44;
              border-radius:6px;padding:.2rem .6rem;font-size:.78rem;font-family:monospace}}
.ml-source-tag{{font-size:.72rem;background:#1a2d1a;color:#3fb950;border:1px solid #3fb95033;
               border-radius:4px;padding:.1rem .35rem;margin-left:.3rem}}
.ml-card-row{{background:#0a1f0a!important;border-left:3px solid #3fb950!important}}
.ml-card-row td.key-col{{color:#3fb950!important}}
</style>
</head>
<body>
...
# ─────────────────────────────────────────────
def push_results_to_mlflow(results: list, ml_agg: dict, sentinel_text: str, stats: dict):
    """
    Push ALL analysis results (Gemini AI + ML pixel metrics) into MLflow.
    Called from main() after analysis completes.
    Creates one run per series + one SENTINEL master run.
    This populates the MLflow UI Experiments view with real data.
    """
    if not HAS_ML_SKILLS:
        return
    try:
        import mlflow as _mlflow
        db_fwd = MLFLOW_DB.replace("\\", "/")
        _mlflow.set_tracking_uri(f"sqlite:///{db_fwd}")
        _mlflow.set_experiment("BrainMRI_Analysis")

        base_tags = {
            "patient":      PATIENT["name"],
            "dob":          PATIENT["dob"],
            "age_at_scan":  str(PATIENT["age"]),
        ...
        for i, r in enumerate(results):
            ...
            desc  = r.get("series_desc", "unknown")
            agent = r.get("agent", "AXIOM")
            ok    = "error" not in r or "raw_analysis" in r
            icon  = {"vessels":"🩸","dwi":"🔬","wmh":"🧠","t1volume":"📐",
                     "t2struct":"🔍","mra_structural":"🧬","mip":"🔭"}.get(role,"📋")

            with _mlflow.start_run(run_name=f"{icon}_{role}_{desc[:30]}_{PATIENT['scan_date']}"):
                _mlflow.set_tags({**base_tags,
                    "section":      f"{icon} {role}",
                    "series_desc":  desc,
                    "agent":        agent,
                    "role":         role,
                    "status":       "OK" if ok else "ERROR",
                    "images_total": str(r.get("total_images_in_series", 0)),
                })
                # Log all string fields as params
                params = {}
                metrics = {}
                for k, v in r.items():
                    if k.startswith("_") or k in ("series_name","series_desc","agent",
                                                    "role","total_images_in_series"): continue
                    if isinstance(v, bool):
                        metrics[k[:250]] = float(v)
                        ...
                        for sk, sv in v.items():
                            full_key = f"{k}_{sk}"[:250]
                            if isinstance(sv, (int, float)):
                                metrics[full_key] = float(sv)
                            elif isinstance(sv, (str, bool)):
                                params[full_key] = str(sv)[:500]
                if params:  _mlflow.log_params(params)
                if metrics: _mlflow.log_metrics(metrics)

                # ML pixel analysis sub-metrics
                ml = r.get("_ml_pixel_analysis", {})
                if ml:
                    ml_metrics = {}
                    ml_params  = {}
                    for section, sdata in ml.items():
                        if isinstance(sdata, dict):
                            for sk, sv in sdata.items():
                                fk = f"ml_{section}_{sk}"[:250]
                                if isinstance(sv, (int, float)):
                                    ml_metrics[fk] = float(sv)
                                elif isinstance(sv, str):
                                    ml_params[fk] = sv[:500]
                    if ml_metrics: _mlflow.log_metrics(ml_metrics)
                    if ml_params:  _mlflow.log_params(ml_params)
                    _mlflow.set_tag("ml_confidence", ml.get("confidence","low"))
                    _mlflow.set_tag("mlflow_pixel_run", ml.get("mlflow_run_id","")[:50])

            print(f"  📊 MLflow: [{agent}] {icon} {role} — {desc[:40]}")

        # ── SENTINEL master run ───────────────────────────────────────────────
        with _mlflow.start_run(run_name=f"🛡️_SENTINEL_MASTER_{PATIENT['scan_date']}"):
            _mlflow.set_tags({**base_tags,
                "section":       "🛡️ SENTINEL Master Report",
                "agent":         "HERMES",
                "role":          "sentinel_coordinator",
                "series_ok":     str(stats.get("series_ok",0)),
                "series_total":  str(stats.get("series_total",0)),
            })
            _mlflow.log_params({
                "series_ok":     str(stats.get("series_ok",0)),
                "series_total":  str(stats.get("series_total",0)),
                "ml_series":     str(stats.get("ml_series",0)),
                "report_html":   str(OUTPUT_HTML),
                "report_txt":    str(OUTPUT_TXT),
                "sentinel_preview": sentinel_text[:500],
            })
            _mlflow.log_metrics({
                "total_images":  float(stats.get("total_images",0)),
                "slices_sent":   float(stats.get("slices_sent",0)),
                "series_ok":     float(stats.get("series_ok",0)),
            })
            # ML aggregate
            t  = ml_agg.get("tissue",{})
            w  = ml_agg.get("wmh",{})
            ...
            mo = ml_agg.get("morph",{})
            agg_metrics = {}
            for k,v in {**t,**w,**m,**mo}.items():
                if isinstance(v,(int,float)):
                    agg_metrics[f"agg_{k}"[:250]] = float(v)
            if agg_metrics: _mlflow.log_metrics(agg_metrics)
        print(f"  📊 MLflow: [HERMES] 🛡️ SENTINEL master run logged")
        print(f"  🌐 View → http://127.0.0.1:5001 → BrainMRI_Analysis experiment")

    except Exception as e:
        print(f"  ⚠️  MLflow push error: {e}")


# ─────────────────────────────────────────────
def main():
    print(f"\n{'='*60}")
    print(f"  🩸 MRA BRAIN ANALYSIS — {PATIENT['scan_date']}")
    print(f"  {PATIENT['name']} | Age {PATIENT['age']}")
    print(f"  Source:  {MRA_ROOT}")
    print(f"  Reports: {OUTPUT_DIR}")
    if HAS_ML_SKILLS:
        print(f"  MLflow:  {MLFLOW_DB}")
        print(f"  🧬 BrainSkillClient: ENABLED (GMM segmentation + MLflow tracking)")
    else:
        print(f"  ⚠️  BrainSkillClient: DISABLED (brain_ml_skills.py not found)")
    print(f"\n  🤖 AGENT MANIFEST:")
    for agent, desc in AGENT_MANIFEST.items():
        print(f"     [{agent}] {desc}")
    print(f"  🤖 HERMES coordinator will route {len(SERIES_ROUTES)} series keyword patterns")
    print(f"{'='*60}\n")
    ...
    # ── Initialise BrainSkillClient (ML + MLflow) ─────────────────────────────
    ml_client = None
    if HAS_ML_SKILLS:
        try:
            ml_client = BrainSkillClient(
                mlflow_db    = MLFLOW_DB,
                patient_name = PATIENT["name"],
                scan_date    = PATIENT["scan_date"],
            )
        except Exception as e:
            print(f"  ⚠️  BrainSkillClient init failed: {e} — continuing without ML")
    ...
    for s in series_list:
        agent = s.get("agent","AXIOM")
        role  = s.get("role","unknown")
        desc  = AGENT_MANIFEST.get(agent, agent)
        print(f"  [{agent}] {s['se_name']} | {s['desc']} | {s['n_images']} imgs → role:{role}")
    print()
    ...
    # Aggregate ML metrics across all series for the panel
    ml_agg = {
        "tissue": {}, "wmh": {}, "mta": {}, "morph": {},
    ...
    for s in series_list:
        print(f"\n🔬 {s['se_name']} — {s['desc']} ({s['n_images']} images)...")
        r = analyse(gemini, s, ml_client=ml_client)
        results.append(r)
        slices_sent += r.get("slices_analysed", 0)
        ok = "error" not in r or "raw_analysis" in r
        print(f"  {'✓' if ok else '✗'} Gemini done — {r.get('slices_analysed',0)} slices sent")

        # Accumulate ML metrics (take first successful mra_structural result)
        ml_data = r.get("_ml_pixel_analysis", {})
        if ml_data and not ml_agg["tissue"]:
            ml_agg["tissue"]   = ml_data.get("tissue", {})
            ml_agg["wmh"]      = ml_data.get("wmh", {})
            ml_agg["mta"]      = ml_data.get("mta", {})
            ml_agg["morph"]    = ml_data.get("morph", {})
            ml_agg["overall_confidence"] = ml_data.get("confidence", "low")
            ml_agg["mlflow_run_id"]      = ml_data.get("mlflow_run_id", "")
        if ml_data:
            ml_agg["n_slices_total"] += ml_data.get("elapsed_s", 0)  # proxy count
            ml_series_count += 1

        time.sleep(8)

    # Count total MLflow runs
    if ml_client and HAS_ML_SKILLS:
        try:
            import mlflow
            runs = mlflow.search_runs(
                experiment_names=["BrainMRI_Analysis"], max_results=500
            )
            ml_agg["total_runs"] = len(runs)
            ml_agg["n_slices_total"] = ml_series_count
        except Exception:
            pass
    ...
    stats = dict(
        ...
        ml_series    = ml_series_count,
    )

    # ── Build ML summary string for SENTINEL prompt ────────────────────────────
    ml_summary = ""
    if ml_agg["tissue"]:
        t, w, m, mo = ml_agg["tissue"], ml_agg["wmh"], ml_agg["mta"], ml_agg["morph"]
        ml_summary = (
            f"GMM Tissue: GM={t.get('grey_matter_pct','?')}% "
            f"WM={t.get('white_matter_pct','?')}% "
            f"CSF={t.get('csf_pct','?')}% "
            f"GM/WM={t.get('gm_wm_ratio','?')} [{t.get('confidence','low')} conf]\n"
            f"WMH: {w.get('wmh_pct_of_wm','?')}% of WM | "
            f"Fazekas PV={w.get('fazekas_periventricular','?')} "
            f"Deep={w.get('fazekas_deep','?')} | "
            f"Burden={w.get('burden','?')} | "
            f"Lacunes={w.get('lacune_candidate_count','?')}\n"
            f"MTA proxy: L={m.get('mta_proxy_left','?')} R={m.get('mta_proxy_right','?')} "
            f"Asym={m.get('asymmetry_pct','?')}% "
            f"HOC L={m.get('hoc_estimate_left','?')}% R={m.get('hoc_estimate_right','?')}%\n"
            f"Morph: AtrophyGrade={mo.get('atrophy_grade','?')} "
            f"Sulcal={mo.get('sulcal_widening','?')} "
            f"Ventricles={mo.get('ventricular_size','?')} "
            f"BrainAgeDelta={mo.get('brain_age_delta','?')}yr"
        )

    # ── SENTINEL master report ─────────────────────────────────────────────────
    print("\n🛡️  SENTINEL v3 compiling master report...")
    master = sentinel_summary(gemini, results, ml_summary=ml_summary)

    # ── Write outputs ──────────────────────────────────────────────────────────
    write_txt(results, master, OUTPUT_TXT, stats)
    write_html(results, master, OUTPUT_HTML, stats,
               ml_stats=ml_agg if ml_agg["tissue"] else None)

    # ── Push ALL results to MLflow UI ─────────────────────────────────────────
    print("\n📊 Pushing all results to MLflow UI...")
    push_results_to_mlflow(results, ml_agg, master, stats)

    # ── Delta comparison from MLflow (only vs different scan dates) ──────────
    if ml_client and ml_agg["tissue"]:
        print("\n📊 MLflow delta vs prior scan:")
        try:
            import mlflow as _mlflow
            all_runs = _mlflow.search_runs(
                experiment_names=["BrainMRI_Analysis"],
                order_by=["start_time DESC"],
                max_results=50,
            )
            # Filter to runs with a different scan_date tag
            curr_date = PATIENT["scan_date"]
            prior_runs = all_runs[
                all_runs.get("tags.scan_date", all_runs.get("tags.mlflow.runName","")) != curr_date
            ] if "tags.scan_date" in all_runs.columns else all_runs.iloc[1:]

            if len(prior_runs) >= 1:
                prev = prior_runs.iloc[0]
                curr = all_runs.iloc[0]
                printed = False
                for metric in ["grey_matter_pct","wmh_pct_of_wm","mta_proxy_left","atrophy_grade"]:
                    ck = f"metrics.{metric}"
                    if ck in curr.index and ck in prev.index:
                        cv, pv = curr[ck], prev[ck]
                        import math
                        if math.isnan(cv) or math.isnan(pv): continue
                        delta = float(cv) - float(pv)
                        arrow = "↑" if delta > 0.01 else ("↓" if delta < -0.01 else "→")
                        print(f"  {metric}: {arrow} {delta:+.3f}")
                        printed = True
                if not printed:
                    print("  (metrics from different scan not yet available for comparison)")
            else:
                print("  (first run for this patient — deltas available after next scan)")
        except Exception as e:
            print(f"  (MLflow delta skipped: {e})")
    ...
    if ml_agg["tissue"]:
        t = ml_agg["tissue"]
        print(f"\n🧬 ML Summary:")
        print(f"   GM={t.get('grey_matter_pct')}% | WM={t.get('white_matter_pct')}% | CSF={t.get('csf_pct')}%")
        print(f"   WMH={ml_agg['wmh'].get('wmh_pct_of_wm')}% | MTA L={ml_agg['mta'].get('mta_proxy_left')} R={ml_agg['mta'].get('mta_proxy_right')}")
        print(f"   Confidence: {ml_agg['overall_confidence'].upper()}")
    print(f"{'='*60}\n")
