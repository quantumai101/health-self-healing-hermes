...
from pydantic import Field, ValidationError, confloat, SecretStr, BaseModel
...
# ── Global Engines (Performance Optimization) ───────────────────────────────
# Instantiating engines at module level avoids overhead per request.
# Note: Presidio engines are thread-safe for inference.
...
def redact_phi_sync(data: Dict[str, Any]) -> str:
    """Synchronous redaction logic to be run in executor."""
    ...
    return anonymizer_engine.anonymize(text=text_data, analyzer_results=results).text
...
async def run_gemini_async(manifest_data: Dict[str, Any]) -> ClinicalAssessment:
    """
    Executes the full pipeline (Redaction + LLM) in a thread pool to prevent 
    blocking the event loop.
    """
    client = genai.Client(api_key=settings.gemini_api_key.get_secret_value())
    
    # Clinical Guidelines for System Prompt
    system_instruction = (
        "You are a clinical decision support assistant. "
        "Analyze PSA trends and lab results against standard urological guidelines. "
        "Prioritize patient safety. If data is ambiguous, state low confidence."
    )
    ...
    try:
        ...
        logger.debug(f"Assessment details: {assessment.assessment}")
