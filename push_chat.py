"""
push_chat.py -- Place in project ROOT. Reads chat.py from pages/ directly,
writes the patched version, then git pushes. No file copy needed.
Run: python push_chat.py
"""
...
if not DEST.exists():
    print(f"ERROR: {DEST} not found. Run from project root.")
    sys.exit(1)
...
def _render_suggested_ops():
    ...
    for i, (label, agent_name, command) in enumerate(SUGGESTED_OPERATIONS):
        with cols[i % 2]:
            if st.button(label, key=f"push_chat_sugg_{i}", use_container_width=True):
                if not _check_authorization(agent_name):
...
# Write directly to pages/chat.py
DEST.write_text(CONTENT, encoding="utf-8")
print(f"OK: Written {DEST} ({DEST.stat().st_size} bytes)")

# Git push
for cmd, desc in [
    (["git", "add", "pages/chat.py"], "Staged"),
    (["git", "commit", "-m", "fix(chat): HTML viewer via components.html + simulation watermark"], "Committed"),
...
print("DONE: Wait 60s for HF rebuild, then re-run CTCA simulation.")